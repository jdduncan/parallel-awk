...
int     nocall = 0;     /* for not treating reduce ops as functions */
...
%token	<i>	ADD MINUS MPI_CALL MULT DIVIDE MOD
%token	<i>	ASSIGN ASGNOP ADDEQ SUBEQ MULTEQ DIVEQ MODEQ POWEQ
%token	<i>	PRINT PRINTF SPRINTF
%token	<p>	ELSE INTEST CONDEXPR
%token	<i>	POSTINCR PREINCR POSTDECR PREDECR
%token	<cp>	VAR IVAR VARNF CALL NUMBER STRING REDUCE
%token	<s>	REGEXPR
...
%left	ARG BLTIN BREAK CALL CLOSE  
%left   CONTINUE DELETE DO EXIT FOR FUNC GSUB IF INDEX LSUBSTR 
%left	MATCHFCN MPI_CALL NEXT NUMBER PRINT PRINTF RETURN SEND SENDRECV 
%left   SPLIT SPRINTF STRING SUB SUBSTR REGEXPR VAR VARNF IVAR 
%left   WHILE '('
%left	CAT
%left	'+' '-'
%left	'*' '/' '%'
%left	NOT UMINUS REDUCE
%right	POWER
%right	DECR INCR
%left	INDIRECT
...
term:
	...
	| MPI_CALL '(' ')'		{ $$ = op2(MPI_CALL, itonp($1), rectonode()); }
	| MPI_CALL '(' patlist ')'	{ $$ = op2(MPI_CALL, itonp($1), $3); }
	| NUMBER			{ $$ = celltonode($1, CCON); }
	| REDUCE {nocall++;} '(' varname {--nocall;} '('  term ')' ')'  
		{ $$ = op2(REDUCE, $4, $7); } 
	| RECV var LT term		{ $$ = op4(SENDRECV, NIL, NIL, $4, $2); }
	| SEND term GT term             { $$ = op4(SENDRECV, $2, $4, NIL, NIL); }
        | SENDRECV '(' term comma term comma var ')'
                { $$ = op4(SENDRECV, $3, $5, $5, $7);   }
        | SENDRECV '(' term comma term comma var comma term ')'
		{ $$ = op4(SENDRECV, $3, $5, $9, $7); } 
